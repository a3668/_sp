...
enum
{ // token : 0-127 直接用該字母表達， 128 以後用代號。
  Num = 128,
  Fun,
  Sys,
  Glo,
...
void next() // 詞彙解析 lexer
{
  ...
  while (tk = *p)
  {
    ...
    { // 取得變數名稱
      ...
      while (id[Tk])
      { // 符號表還未結束
        if (p - pp == id[Len] && !memcmp((char *)id[Name], pp, p - pp))
        {
          tk = id[Tk];
          return;
        } // 找到就傳回該符號的 token
    ...
    { // 字元或字串
      ...
      if (tk == '"')
        ...
        tk = Num; // (若是字串) ? (ival = 字串 (在資料段中的) 指標) : (字元值)
      return;
    } // 以下為運算元 =+-!<>|&^%*[?~, ++, --, !=, <=, >=, ||, &&, ~  ;{}()],:
    else if (tk == '=')
    {
...
void expr(int lev) // 運算式 expression, 其中 lev 代表優先等級
{
  ...
  { // 處理 sizeof(type) ，其中 type 可能為 char, int 或 ptr
    next();
    if (tk == '(')
      next();
    else
    ...
    *++e = IMM;
    *++e = (ty == CHAR) ? sizeof(char) : (ty == INT) ? sizeof(int)
                                                     : sizeof(char *);
    ty = INT;
  }
  else if (tk == Id)
  { // 處理 id ...
    ...
    { // id (args) ，這是 call
      ...
      else if (d[Class] == Fun)
      {
        *++e = JSR;
        *++e = d[Val];
      } // token 是自訂函數，用 JSR : jump to subroutine 指令呼叫
      else
      {
    ...
    else
    {
      if (d[Class] == Loc)
      {
        *++e = LLA;
        *++e = loc - d[Val];
      } // 該 id 是區域變數，載入區域變數 (LLA : load local address)
      else if (d[Class] == Glo)
      {
        *++e = IMM;
        *++e = d[Val];
      } // 該 id 是全域變數，載入該全域變數 (IMM : load global address or immediate 載入全域變數或立即值)
      else
      {
...
void stmt() // 陳述 statement
{
  ...
  else if (tk == Do)
  {
    ...
    { // 確保 do-while 有 while
      ...
      *++e = BNZ; // 如果條件為真，跳回迴圈起點 BNZ : branch if not zero
      *++e = (int)a;
    }
    else
    {
  ...
  { // for (exp1; exp2; exp3) stmt
    ...
    *++e = BZ;
    a = ++e; // 條件為假時跳出循環 (a 記住跳耀為止)

    *++e = JMP;
    b = ++e; // 跳到 stmt (b 記住跳耀位址)
      ...
      { // function 函數定義 ex: int f( ...
        ...
        { // 掃描參數直到 ...)
          ...
          if (id[Class] == Loc)
          {
            ...
            return -1;
          } // 這裡的 id 會指向 hash 搜尋過的 symTable 裏的那個 (在 next 裏處理的)，所以若是該 id 已經是 Local，那麼就重複了！
...
int compile(int argc, char **argv) // 主程式
{
  ...
  p = "char else enum if int return sizeof for do while " // 放入符號表
