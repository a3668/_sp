...
這是一個使用 C 語言實作的 CHIP-8 模擬器  
CHIP-8 是一個簡單的虛擬機器，最初設計用於在 1970 年代的計算機上運行遊戲。
...
- 這函式的作用是接收一個 SDL 鍵盤事件 e 和 Chip-8 模擬器狀態 chip8
- 再來判斷是按下還是放開
...
- 失敗會輸出錯誤訊息和清除已初始化的部分
- 成功後為視窗建立一個渲染器
  ![creatrender重點](./pic/CreateRenderer.png)
